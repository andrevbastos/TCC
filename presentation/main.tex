...
\titlegraphic{\hfill\includegraphics[scale=0.2]{figuras/ifc-logo.png}}
...
\begin{frame}{Malhas 3D no Motor IFCG}
    \begin{columns}
        ...
        \begin{column}{0.6\textwidth}
            \centering
            \only<1>{\includegraphics[width=0.85\textwidth]{figuras/vertices.png}}
            \only<2>{\includegraphics[width=0.85\textwidth]{figuras/arestas.png}}
            \only<3>{\includegraphics[width=0.85\textwidth]{figuras/faces.png}}
            \only<4>{\includegraphics[width=0.85\textwidth]{figuras/malha.png}}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Resultados Parciais --- Experimento de Escala}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \footnotesize
            \textbf{Análise do Tempo de Execução (ms):}
            \begin{itemize}
                \item \textbf{A* Modificado:} Apresentou o pior desempenho em grandes dimensões, atingindo $13816.9 \text{ ms}$ em malhas de $800\times800$ vértices.
                \item \textbf{JPS:} Mais eficiente em escalas menores, porém degradou nas maiores ($4175.0 \text{ ms}$ em $800\times800$). Isso decorre de buscas longas e sem pontos de salto em relevos suaves.
                \item \textbf{A* e Theta*:} Mantiveram-se mais consistentes ao longo do crescimento da escala.
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \centering
            \includegraphics[width=\textwidth]{figuras/plot_escala_tempo_tendencia.png}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Resultados Parciais --- Análise Estatística}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \footnotesize
            \textbf{Validação Estatística (ANOVA + Tukey):}
            \begin{itemize}
                \item A ANOVA de duas vias confirmou que a escolha do Algoritmo, da Escala e sua interação possuem relevância estatística sobre o tempo ($p \approx 0$).
                \item O teste pós-hoc de Tukey HSD ratificou a significância estatística das diferenças nas escalas maiores ($p < 0.05$).
                \item \textbf{Lacunaridade e Relevo:} Em alta frequência de relevo (lacunaridade $\ge 2.0$), o JPS superou amplamente os demais (médias de $2.4 \text{ ms}$).
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \centering
            \includegraphics[width=\textwidth]{figuras/plot_escala_nos_tendencia.png}
        \end{column}
    \end{columns}
\end{frame}
